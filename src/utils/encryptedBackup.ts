...
export const WRONG_PASSPHRASE_MESSAGE = 'Could not decrypt this backup. The passphrase is wrong or the file is corrupted.'
...
export async function encryptBackupJson(
  ...
  const iterations = Math.max(1, Math.floor(options.iterations ?? DEFAULT_PBKDF2_ITERATIONS))
  const salt = globalThis.crypto.getRandomValues(new Uint8Array(16))
  const iv = globalThis.crypto.getRandomValues(new Uint8Array(12))
  const key = await deriveAesKey(passphrase, salt, iterations)
  const ciphertext = await globalThis.crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv as BufferSource }, key, new TextEncoder().encode(plainJson))
  return {
    app: 'FlightLog',
...
export async function decryptBackupEnvelope(envelope: EncryptedBackupEnvelope, passphrase: string): Promise<string> {
  ...
  try {
    const key = await deriveAesKey(passphrase, base64ToBytes(envelope.kdf.salt), envelope.kdf.iterations)
    const plaintext = await globalThis.crypto.subtle.decrypt(
      { name: 'AES-GCM', iv: base64ToBytes(envelope.cipher.iv) as BufferSource },
      key,
