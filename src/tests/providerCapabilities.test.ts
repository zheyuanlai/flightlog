...
describe('provider capabilities client', () => {
  ...
  it('fetches and normalizes a real capabilities response', async () => {
    const fetcher = async () => new Response(JSON.stringify({ provider: 'aerodatabox', mode: 'real', supportsFlightStatus: true, supportsAirportStatus: false }), { status: 200 })
    const result = await fetchProviderCapabilities({ baseUrl: 'https://worker.example', fetcher: fetcher as unknown as typeof fetch })
    expect(result).toEqual({ provider: 'aerodatabox', mode: 'real', supportsFlightStatus: true, supportsAirportStatus: false })
  })

  it('normalizes a malformed payload to the fail-open defaults', () => {
    ...
    expect(normalizeProviderCapabilities({ provider: 'flightaware', supportsFlightStatus: false })).toEqual({
      provider: 'flightaware', mode: 'unknown', supportsFlightStatus: false, supportsAirportStatus: true,
    })
  })
})
